...
function binaryMax(input) {
  //tìm xem có nhiều nhất bao nhiêu số 1 đứng cạnh nhau -> count
  let count = 0;
  let temp_count = 0;
  for (let i = 0; i < input.length; i++){
    if (input[i] === '1'){
  ...
  //in chuỗi kết quả là chuỗi gồm các số 1 có độ dài count
  let string_result = '';
...
function uniqueArr(arr) {
  ...
  for (let i = 0; i < sorted_arr.length; i++){
    if(!unique_arr.includes(sorted_arr[i])){
...
function mergeArr(arr1, arr2) {
  ...
  for (let i = 0; i < arr2.length; i++){
    arr_result.push(arr2[i]);
  }
  arr_result = uniqueArr(arr_result);
  return arr_result;
}
