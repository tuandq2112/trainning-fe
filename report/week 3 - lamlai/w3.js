...
function sumNumber(input) {
   ...
   var result = arr.reduce((sum,currentValue)=>{
...
function sumVowel(input) {
  ...
  var arr = input.split('');
  console.log(arr);
  var arrCount = arr.filter((value)=>{
    if(nguyen_am.includes(value)){
      return value;
    }
  })
...
function uniqueArr(arr) {
  ...
  const sortedSet = new Set(sorted_arr);
  const resultArray = [...sortedSet];
  return resultArray;
}
